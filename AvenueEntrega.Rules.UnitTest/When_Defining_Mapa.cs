...
namespace AvenueEntrega.Rules.UnitTest
{
    ...
    public class When_Defining_Mapa
    {
        ...
        {
            DomainServices services = new DomainServices(this._testMapa);

            var path = services.GetBestPath("A", "D");
        ...
        public void Test3_Lower_Cost_Route_From_A_To_D_Will_Pass()
        {
            ...
            var state = services.ChiepestFirstSearch("Arad", "Bucharest");

            var custo = services.GetCostForBestPath("A", "D",10, 2.50f);
        ...
        {
            DomainServices services = new DomainServices(this._romeniaMapa);

            var path = services.GetBestPath("Arad", "Bucharest");
